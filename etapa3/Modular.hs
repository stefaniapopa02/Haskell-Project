...
-}
mapSingle :: (a -> a) -> [a] -> [[a]]
mapHelper hd tl f = if (null tl) 
                         then []
                         else  (hd++[f (head tl)]++(tail tl)):(mapHelper (hd++[head tl]) (tail tl) f)

mapSingle f xs = mapHelper [] xs f
...
partitions :: [a] -> [[[a]]]
partitions [] = [[]]
partitions (x:xs) = [new |rest <- partitions xs,  new <- ([x]: rest) : (mapSingle (x:) rest)]
...
helperIM node graph set = let inN = S.difference (inNeighbors node graph) set
                              outN = S.difference (outNeighbors node graph) set
                          in (inN, outN)
...
isModule :: Ord a
         ...
         -> Graph a
         -> Bool
isModule set graph = if (length $ S.map (\x -> helperIM x graph set) set) == 1 then True else False
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = (length $ S.filter (\x -> isModule x graph) partition) == (length partition)
...
maximalModularPartition :: Ord a
                        ...
                        -> Graph a
                        -> Partition a
maximalModularPartition partitions graph = minimumBy (compare `on` length) (S.filter (\x->isModularPartition x graph) (S.difference (S.fromList partitions) (S.fromList [(S.fromList [nodes graph])])))
