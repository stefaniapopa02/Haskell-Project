...
selectNeighbors neighb vis = if null neighb 
                                then []
                                else if (notElem (head neighb) vis)
                                        then (head neighb) : selectNeighbors (tail neighb) vis
                                        else selectNeighbors (tail neighb) vis


helperFunc nodesToAnalyze graph visited f =  if (not (null nodesToAnalyze)) 
                                                then (let first = head nodesToAnalyze in 
                                                        (if (elem first visited) 
                                                            then helperFunc (tail nodesToAnalyze) graph visited f
                                                            else first :
                                                            helperFunc (f (tail nodesToAnalyze) (selectNeighbors (S.toList $ outNeighbors first graph) visited)) 
                                                            graph (first : visited) f))
                                                else []



search f node graph = helperFunc [node] graph [] f
...
bfs node graph = search (\nodesToAnalyze neighbors -> nodesToAnalyze ++ neighbors) node graph
...
dfs node graph = search (\nodesToAnalyze neighbors -> neighbors ++ nodesToAnalyze) node graph
...
helperCount from to graph f = (length $ fst $ span (/= to)  (f from graph)) - 1

countIntermediate from to graph = if (elem to (bfs from graph)) 
                                    then Just (helperCount from to graph bfs, helperCount from to graph dfs)
                                    else Nothing
